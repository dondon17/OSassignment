...
pick_next:
	...
	if (!list_empty(&readyqueue)) {
		list_for_each_entry_safe(cursor, next, &readyqueue, list){
...
pick_next:
	...
	if (!list_empty(&readyqueue)) {
		list_for_each_entry_safe(cursor, next, &readyqueue, list){
			...
			else if(cursor->lifespan == min){
				if(temp->age < cursor->age){
					temp=cursor;
				}
			}
		}

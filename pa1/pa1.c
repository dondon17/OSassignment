...
// cd command
	if(strncmp(tokens[0], "cd", strlen("cd"))==0){
        char *path;
        if(nr_tokens>1){
            if(strcmp(tokens[1], "~")==0) path=(char*)getenv("HOME"); 
            else path=tokens[1];
        }
        else if((path=(char*)getenv("HOME"))==NULL) path=".";
        if(chdir(path)==-1) fprintf(stderr, "No such directory\n");
		return 1;
    }
...
// prompt command
    if(strncmp(tokens[0], "prompt", strlen("prompt"))==0){ 
        ...
        __prompt[sizeof(tokens[1])]='\0';
		return 1;
    }

// for command
	if(strncmp(tokens[0], "for", strlen("for"))==0){
		for(int i=0;i<atoi(tokens[1]);i++) run_command(nr_tokens-2, tokens+2);	
		return 1;
	}
